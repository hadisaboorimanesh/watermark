...
def add_watermark(directory, logo_path, position, new_directory, padding, scale):
    ...
    #download_folder(minio_client, bucket_name, directory, new_directory,file_formats)
    """
    ...
    """
    print(os.path.basename(logo_path))
    EXTS = ('.jpg', '.jpeg', '.png')
        ...
        for filename in filenames:
            if filename.lower().endswith(EXTS) and filename != os.path.basename(logo_path):
               ...
               # if image.mode == 'RGBA':
                     #print('hadi')
                     #print(filenames)
                     #image_mask_original = image.split()[3]
                     #print(image_mask_original)
                #else:
                 #    image_mask_original = None
                ...
                try:
                     ...
                     print(f"An error occurred: {e} {filename}")
                finally:
